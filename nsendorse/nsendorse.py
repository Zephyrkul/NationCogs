...
class NSEndorse:
    ...
    async def ne(self, ctx, *, wanation):  # API requests: 1; non-API requests: 0
        ...
        await self._file(ctx.message.channel, self._endocheck(await self.nsapi.api("endorsements", "wa", nation=wanation).collect())["endorsements"].replace(",", self.delim), "ne")
    ...
    async def nec(self, ctx, *, wanation):  # API requests: 1; non-API requests: 0
        ...
        self._checks(ctx.prefix)
        await self.bot.say(self._endocheck(await self.nsapi.api("censusscore-66", "wa", nation=wanation))["censusscore"]["text"])
    ...
    async def nne(self, ctx, *, wanation):  # API requests: 3; non-API requests: 0
        ...
        endos = self._endocheck(
            await self.nsapi.api("endorsements", "region", "wa", nation=wanation))
    ...
    async def nnec(self, ctx, *, wanation):  # API requests: 3; non-API requests: 0
        ...
        self._checks(ctx.prefix)
        endos = self._endocheck(
            await self.nsapi.api("censusscore-66", "region", "wa", nation=wanation))
    ...
    async def spdr(self, ctx, *, nation):  # API requests: 1; non-API requests: 0
        ...
        self._checks(ctx.prefix)
        await self.bot.say(await self.nsapi.api("censusscore-65", nation=nation).collect()["censusscore"]["text"])
